...
def non_linear_simulation(t_max, dt, initial_position, k_0, k_lin, k_cub, k_crossed, gamma):
    ...
    dt               : time increment
    initial_position : initial condition
    k_0              : constant term        of the potential
    k_lin            : linear term constant of the potential
    k_cub            : cubic  term constant of the potential
    ...
    positions : array with simulated positions
    """
    global k_B , T, N_time, N_dimensions, F_scatt_amplitude
...
def V_cub(X, Y, Z, k_0, k_lin, k_cub, k_cross):
    """
    Non-linear potential 
    ...
    V =  k_0 + k_lin[0]*X**2 + k_lin[1]*Y**2 + k_lin[2]*Z**2 + k_cub[0]*X**4 + k_cub[1]*Y**4 + k_cub[2]*Z**4 + 2.0*k_cross*(X**2)*(Y**2)
    return V

# Linear potential
def V_lin(X, Y, Z, k_0, k_lin):
    """
    Non-linear potential 
    ...
    V =  k_0 + k_lin[0]*X**2 + k_lin[1]*Y**2 + k_lin[2]*Z**2
    return V
###############################################################################
...
Z = np.zeros( X.shape )                        # Only look on the z=0 plane
...
# Extra parameters if we wish to consider the Scattering Force
...
k_cub_x = (0.5*1e14) * k_lin_x                    # Non-linearity strengths on the xy plane [N/m^4]
...
N_examples = 300                                    # Number of examples to generate
...
potential_lin = V_lin(X, Y, Z, 0, k_lin)
potential_cub = V_cub(X, Y, Z, 0, k_lin, k_cub, k_cross)
